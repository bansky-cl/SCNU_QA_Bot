...
def get_answer(query, vs_path , history, mode = "知识库问答", score_threshold=VECTOR_SEARCH_SCORE_THRESHOLD,
        ...
        for resp, history in local_doc_qa.get_knowledge_based_answer(
            ...
            source += "".join(
                [f"""<details> <summary>出处 [{i + 1}] {os.path.split(doc.metadata["source"])[-1]}</summary>\n"""
                 f"""相关度：{round((doc.metadata['score']) / 1100, 4)}\n\n""" # 相似度
                 f"""{doc.page_content}\n""" #  这个page_content是被chunk过的，CHUNK_SIZE默认是250，在config.py
                 f"""</details>"""
                 for i, doc in
...
def get_vector_store(vs_id, files, sentence_size, history, one_conent, one_content_segmentation):
    ...
    vs_path, _ = local_doc_qa.init_knowledge_vector_store(vs_path, sentence_size)
        # else:
...
🚩本项目基于[langchain-ChatGLM](https://github.com/imClumsyPanda/langchain-ChatGLM)开发   


# 模型说明v4.0🎈update:5.20
- 经收集了2014年2023.5.17的文章2200余篇，已全部爬取，知识库仅做学术研究   

# 重要声明🎇
1. 模型生成信息不代表作者立场，请谨慎甄别有害的虚假信息
2. 我们坚定认为香港、澳门、台湾是中华人民共和国的一部分。坚决维护中国主权完整和用户中国共产党的领导
3. 本项目属于学术研究性质，不参与商用   

# 联系开发者😎
@ [banskyc](https://github.com/bansky-cl)   

# 历史更新⏰ 
- update:5.17 模型说明v3.0，已经收集了2017.2.23到2023.5.13的文章约1230篇，目前文章爬取进度64%
- update:5.13 模型说明v2.0，知识库来源是SCNU的官方公众号《晚安华师》，收集了2019.12.25到2023.5.13的文章约750篇，目前文章爬取进度40%

"""
default_vs = vs_list[0] if len(vs_list) > 1 else "为空"
init_message = f"""欢迎使用 SCNU-QA Web UI！

当前知识库{default_vs}, 可开始提问，尽量提问与华师相关内容
...
with gr.Blocks(css=block_css) as demo:
    ...
    # files = os.listdir('content/0518_9k3')
    ...
    #     print(q)
    #     print(type(q))
    #     local_doc_qa.get_knowledge_based_answer(query=q, vs_path=vs_path, streaming=True)
    ...
    with gr.Row():
        with gr.Column(scale=10):
            ...
            query.submit(get_answer,[ query, vs_path, chatbot],[chatbot, query])
